...
impl Distribution<EVM> for rand::distributions::Standard {
    fn sample<R: Rng + ?Sized>(&self, rng: &mut R) -> EVM {
        let inner: Inner = rng.gen();
        inner.into()
    }
}

impl std::str::FromStr for EVM {
...
impl SynthLanguage for Math {
    ...
    /// Converting CVC4's rewrites to Ruler's EVM grammar syntax.
    ...
    fn init_synth(synth: &mut Synthesizer<Self>) {
        let mut consts: Vec<Option<EVM>> = vec![];
        if synth.params.complete_cvec {
            ...
            for i in 0..synth.params.important_cvec_offsets {
                let i = EVM::from(i);
                consts.push(Some(EVM::MIN.wrapping_add(i)));
                consts.push(Some(EVM::MAX.wrapping_sub(i)));
                consts.push(Some(i));
        ...
        for row in consts.iter_mut() {
            ...
            let vals = std::iter::repeat_with(|| synth.rng.gen::<EVM>());
        ...
        // egraph.add(Math::Num(EVM::ZERO));
        // egraph.add(Math::Num(1.into()));
        // egraph.add(Math::Num(2.into()));
        // egraph.add(Math::Num(EVM::MIN));
        // egraph.add(Math::Num(EVM::MAX));
